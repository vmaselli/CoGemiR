...
=head2 new
 ...
 Title          : new
 Usage          : my $loader =  Bio::Cogemir::LoadDB->new(   
							           -driver      => $driver,
							           -host        => $host,
							           -user        => $user,
							           -port        => $port,
...
sub new{
	...
	$port &&   $self->port($port);
	$pass &&   $self->pass($pass);
	$schema_sql && $self->schema_sql($schema_sql);
	$module && $self->module($module);
	$dbname && $self->dbname($dbname);
...
sub create_db {
    ...
    eval{$db->do("DROP DATABASE $db_name ")};
