...
my $query = Bio::DBLoader->new(-settings => $settings,
								                    -dbh =>$dbh);		
...
my $sql = qq{INSERT INTO external_database SET micro_rna_id = ?, database_name = ?, accession_number = ?, display = ?};
...
# define the clade for each species
...
my $log_file = "$ENV{'HOME'}/src/cogemir-49/log/log.txt";
open (LOG, ">>$log_file");

# open the miRBase file to parse them
my $files_dir = "$ENV{'HOME'}/Projects/mirna/data/mirbase/";
...
open (MCB, $mirna_location) || die $!;
open (M2F, $mirna2fam) || die $!;
open (MPF, $mirna_prefamtxt) || die $!;
open (SP, $speciestxt) || die $!;
open (ED, $externaldbtxt) || die $!;
...
while (my $mir_row = <MIR>){
  ...
  my ($prefam_id,$famacc,$fam_name,$fam_description) = split /\t{1,10}/,$prefam_row;
  my $param = $famacc;
  my @groups = split /\-/, $mirna_name; #hsa-mir-200b
  shift @groups;
  if (scalar @groups == 3){pop @groups}
  my $name = join ("-",(@groups)); #mir-200b
  unless($fam_name){$fam_name = $name};
  unless ($param){$param = 'none';$name = substr($mirna_name,4);}
  ...
  # MICRO RNA OBJ 
  # create the obj for the generic microRNA e.g. 'mir-204'
  ...
  # create the attribute obj wich contain the main information about the gene (both microRNA
  # and hostgene, such as the name, the location and so on
  ...
  my $host = $query->genoinfo->search_host_in_ensembl_by_slice($host_genome_db,$location); 
  #unless (defined $host){warn("host for $mirna_name not defined in ensembl")} 
  my $flanking_gene;
   
  unless ($host){
    $host_genome_db = $genomes->{'vega'}{$species};
    $host = $query->genoinfo->search_host_in_ensembl_by_slice($host_genome_db,$location);
    unless ($host){
  ...
  foreach my $coord_row (@coos){
		...
		my $ens_mirna = $query->genoinfo->search_mirna_in_ensembl_by_slice($genome_db,$location);
		...
		foreach my $hash (@hashes){
  		...
  		$sth->execute($micro_rna_id, $hash->{'database'},$hash->{'accession'},$hash->{'display'});
  	}
  }
  my ($ens_tr,$ens_p);
  if ($host){
...
#                 );  
  ...
  if (defined $gene){
  	my $genome_db = $genomes->{'core'}{$species};
    my $slice = $query->genoinfo->search_in_ensembl_by_slice($genome_db,$location);
    ...
    my $vega = $query->genoinfo->search_host_in_ensembl_by_slice($vega_genome_db,$location);
    
    my $est_genome_db = $genomes->{'otherfeatures'}{$species};
    my $est = $query->genoinfo->search_host_in_ensembl_by_slice($est_genome_db,$location);
    ...
    if ($direction eq 'sense'){
      ...
      my $reverse_gene = $query->genoinfo->search_host_in_ensembl_by_slice($genome_db,$rev_location);
...
sub _search_flanking_region{
  ...
  while ($window < 30001){
    ...
    my $upslice = $query->genoinfo->search_in_ensembl_by_slice($genome_db,$up_location);
  ...
  while ($window < 30001){
    if ($newstart < 0 || $newend < 0){last}
    my $downslice = $query->genoinfo->search_in_ensembl_by_slice($genome_db,$down_location);
